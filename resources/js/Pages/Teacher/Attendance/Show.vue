...
const updateAttendance = async (studentId, date, status) => {
    try {
        await axios.post(route('teacher.attendance.store'), {
...
const markAllForDate = async (date) => {
    ...
    try {
        ...
        await axios.post(route('teacher.attendance.bulk-store'), {
            group_id: props.group.id,
            date: date,
...
const previousWeek = () => {
    ...
    router.get(route('teacher.attendance.show', props.group.id), { date: date.toISOString().split('T')[0] });
};
...
const goToToday = () => {
    router.get(route('teacher.attendance.show', props.group.id), { date: new Date().toISOString().split('T')[0] });
};
...
const saveAttendanceDays = async () => {
    ...
    try {
        await axios.post(route('teacher.attendance.update-attendance-days', selectedStudent.value.id), {
...
const saveBulkAttendanceDays = async () => {
    ...
    try {
        await axios.post(route('teacher.attendance.bulk-update-attendance-days', props.group.id), {
