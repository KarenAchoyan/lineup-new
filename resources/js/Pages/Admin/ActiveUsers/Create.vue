<template>
    <AuthenticatedLayout>
        <Head title="Create Active User" />

        <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                    <div class="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                        <h2 class="text-xl font-semibold text-gray-900">Create Active User</h2>
                    </div>
                    ...
                    <form @submit.prevent="submit" class="p-6 space-y-6">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4">
                            ...
                            <div v-if="form.errors.about_en || form.errors.about_hy || form.errors.about_ge || form.errors.about_ru" class="mt-2">
                                <div v-if="form.errors.about_en" class="text-sm text-red-600">{{ form.errors.about_en }}</div>
                                <div v-if="form.errors.about_hy" class="text-sm text-red-600">{{ form.errors.about_hy }}</div>
                                <div v-if="form.errors.about_ge" class="text-sm text-red-600">{{ form.errors.about_ge }}</div>
                                <div v-if="form.errors.about_ru" class="text-sm text-red-600">{{ form.errors.about_ru }}</div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="file" @input="form.avatar = $event.target.files[0]" accept="image/*" required class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-[#F15A2B] file:text-white hover:file:bg-[#BF3206]" />
                        ...
                        <div class="flex items-center justify-end gap-4 pt-4 border-t">
                            ...
                            <button type="submit" :disabled="form.processing" class="px-6 py-2 bg-[#F15A2B] text-white rounded-lg hover:bg-[#BF3206] disabled:opacity-50">
                                {{ form.processing ? 'Creating...' : 'Create Active User' }}
                            </button>
                        </div>
                    </form>
                </div>
...
const submit = () => {
    ...
    form.post(route('admin.active-users.store'), {
        forceFormData: true,
    });
};
</script>
